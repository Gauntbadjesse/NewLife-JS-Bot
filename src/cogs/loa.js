...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'start') {
            await this.startLOA(interaction);
        } else if (subcommand === 'end') {
            await this.endLOA(interaction);
        } else if (subcommand === 'view') {
            await this.viewLOA(interaction);
        }
    },

    async startLOA(interaction) {
        ...
        // Schedule auto-end
        this.scheduleAutoEnd(interaction.client, interaction.guild.id, interaction.user.id, endDate);
    },

    async endLOA(interaction) {
    ...
    async viewLOA(interaction) {
        ...
        for (const loa of activeLOAs) {
            try {
                ...
                const timeLeft = this.getTimeRemaining(loa.endDate);
    ...
    scheduleAutoEnd(client, guildId, userId, endDate) {
    ...
    async checkExpiredLOAs(client) {
        ...
        for (const loa of upcomingLOAs) {
            this.scheduleAutoEnd(client, loa.guildId, loa.userId, loa.endDate);
        }
